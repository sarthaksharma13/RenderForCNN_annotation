...
#g_shapenet_root_folder = os.path.join('/home', 'rishabh', 'RenderForCNN')
...
g_shapenet_root_folder = os.path.join('/media', 'data', 'junaid','Auto','GrabCAD','GrabCADCore.v1')
g_pascal3d_root_folder = os.path.join('/media', 'data', 'data', 'datasets', 'PASCAL3D')
g_sun2012pascalformat_root_folder = os.path.join('/media', 'data', 'data', 'datasets', 'SUN2012pascalformat')
# ------------------------------------------------------------
...
#                              ('03211117', 'tvmonitor'),
#                              ('16155064', 'Auto']
#g_shape_synset_name_pairs = [('03642806', 'laptop')]
#g_shape_synset_name_pairs = [('03001627', 'chair')]
g_shape_synset_name_pairs = [('16155064', 'Auto')]
#g_shape_synset_name_pairs = [('02924116', 'Bus')]
g_shape_synsets = [x[0] for x in g_shape_synset_name_pairs]
g_shape_names = [x[1] for x in g_shape_synset_name_pairs]

g_syn_images_folder = os.path.join('/media','data','junaid','Auto','syn_images')
#g_syn_images_folder = os.path.join(g_data_folder, 'syn_images')
#g_syn_images_cropped_folder = os.path.join('/tmp','rishabh','syn_images_cropped')
g_syn_images_cropped_folder = os.path.join(g_data_folder, 'syn_images_cropped')
#g_syn_images_bkg_overlaid_folder = os.path.join('/tmp','rishabh','syn_images_cropped_bkg_overlaid')
...
g_syn_images_num_per_category = 3
...
g_syn_images_lmdb_folder = os.path.join('/tmp','rishabh','syn_lmdbs')
...
# -----------------------------
# Global variables added by KM
# -----------------------------

# # Directory in which the customized version is located (where we annotated a few chairs with 3D keypoints)
# g_shapenet_chair_custom_folder = os.path.join(g_shapenet_root_folder, '..', 'ShapeNet_Object_SLAM', 'chair_models')
# # Directory where the images (whose 2D keypoint annotations will also be synthesized) are stored
# g_syn_images_keypoints_folder = os.path.join(g_data_folder, 'syn_images_keypoints')
# # Path to the text file where keypoint annotations (on 3D models) are stored
# g_shapenet_chair_keypoint_annotation_file = os.path.join(g_data_folder, 'chair_annotations_3d.txt')
# # Path to the directory where text files corresponding to the keypoint annotations (2D keypoints) are stored
# g_syn_annotations_folder = os.path.join(g_data_folder, 'syn_keypoint_annotations')
