...
#text_file = open("/home/rishabh/ListObjFile1.txt", "r")
# shape_md5_list = text_file.read().split(',')
#text_file.close()
#shape_md5_list.pop(1)
    shape_md5_list = os.listdir(os.path.join(g_shapenet_root_folder,shape_synset))
    view_num = 3 #g_syn_images_num_per_category / len(shape_md5_list)
    ...
    print(os.path.join(g_shapenet_root_folder, shape_synset, shape_md5, 'model.obj'))
...
def load_one_category_shape_views(synset):
    ...
    if not os.path.exists(os.path.join(g_data_folder, g_view_distribution_folder, 'Auto.txt')):
        print('Failed to read view distribution files from %s for synset %s' % 
              os.path.join(g_data_folder, g_view_distribution_folder, 'Auto.txt'))
        exit()
    view_params = open(os.path.join(g_data_folder, g_view_distribution_folder, 'Auto.txt')).readlines()
